    ...
    try:
        ...
        if book.child_id:
            ...
            if child:
                ...
                logger.info(f"📸 Найден ребенок для книги: {child.name}, возраст {child.age} лет, интересы: {child.interests}, характер: {child.personality}")
...
2. Учитывай интересы, характер и особенности ребенка при описании визуальных элементов.
3. Персонаж должен отражать индивидуальность ребенка из анкеты.
...
1. ВОЗРАСТ (КРИТИЧЕСКИ ВАЖНО!): Главный персонаж (ребенок) ДОЛЖЕН выглядеть ТОЧНО как ребенок {child_profile['age']} лет. 
   ...
   - В промпте ОБЯЗАТЕЛЬНО укажи: 'a {child_profile['age']}-year-old child', 'child aged {child_profile['age']}', 'ребенок {child_profile['age']} лет'
   - НИКОГДА не делай персонажа старше или взрослее! Он должен выглядеть именно как {child_profile['age']}-летний ребенок!
  ...
  "prompts": [
    ...
    {{
      ...
      "prompt": "..."
        ...
        db.commit()
        logger.info(f"✓ _create_image_prompts_internal: Промпты сохранены в БД для book_id={request.book_id}, обновлено сцен: {len(updated_scenes)}")
