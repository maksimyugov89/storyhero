#!/usr/bin/env python3
"""
Скрипт для генерации финальных изображений для конкретной книги по book_id.
"""
import sys
import asyncio
...
async def generate_final_images_for_book(book_id: str):
    """Генерирует финальные изображения для конкретной книги."""
    db = SessionLocal()
    ...
    try:
        ...
        try:
            result = await _generate_final_images_internal(
                book_id=str(book.id),
                db=db,
                current_user_id=book.user_id or "test_user",
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        logger.error("❌ Использование: python3 generate_final_images_for_specific_book.py <book_id>")
        sys.exit(1)
    
    book_id = sys.argv[1]
    
    try:
        exit_code = asyncio.run(generate_final_images_for_book(book_id))
        sys.exit(exit_code)
